...
@Injectable()
export class ChangeUnitStatusUseCase {
  constructor(
  ...
  async execute(
    ...
    try {
      ...
      if (!unit.canTransitionTo(targetStatus)) {
        throw new BadRequestException(
          `Cannot transition unit from '${unit.status}' to '${targetStatus}'.`,
        );
      }

      const transitioned = unit.transitionTo(targetStatus);
      ...
      DatabaseErrorHandler.handle(error, 'ChangeUnitStatusUseCase');
    }
  }
}
